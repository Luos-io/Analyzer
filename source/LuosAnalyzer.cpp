        ...
        for (;;)
        {
            ...
            switch (state)
            {
                ...
                case SIZE:
                {
                    ...
                    ComputeCRC(data_byte);
                    if (size == 0)
                        state = CRC; //if no data -> go to crc state
                    else
                        state = DATA;
                    data_byte = 0;
                    break;
        ...
        for (;;)
        {
            ...
            switch (state)
            {
                case PROTOCOL:
                {
                    ...
                    if (!Rx_msg)
                    { //when data exist in channel Tx, move the Tx pointer
                        mTx->Advance(samples_to_first_center_of_first_data_bit);
                        starting_sample += samples_per_bit;
                        if (!mTx->WouldAdvancingCauseTransition(timeout * samples_per_bit))
                        { //long period with no data?->reset
                            transmission_error = 1;
                ...
                case CRC:
                {
                    ...
                    { //msg in Tx
                        ...
                        for (U32 i = 0; i < 16; i++)
                        {
                            ...
                            if ((!mTx->WouldAdvancingCauseTransition(timeout * samples_per_bit)) && i <= 8)
                            { //if no data for timeout period - error
                                mResults->AddMarker(mTx->GetSampleNumber(), AnalyzerResults::ErrorX, mSettings->mRxChannel);
                                mResults->AddMarker(mTx->GetSampleNumber(), AnalyzerResults::ErrorX, mSettings->mTxChannel);
                                transmission_error = 1;
                                break;
                ...
                case ACK:
                {
                    ...
                    else //ack to the other channel => if msg in Rx -> ack to Tx / if msg in Tx -> ack to Rx
                    {
                        ...
                        {
                            if (target == 0)
                            { //if target = 0 and message to Rx -> ack to Rx
                                mRx->AdvanceToNextEdge();
                                mRx->Advance(samples_to_first_center_of_first_data_bit);
            ...
            if (collision_detection && Rx_msg && state == WAIT)
            { //end of message
                ack = 0;
            }
            if (!noop)
            { //if noop=1 ->send no frame
                Frame frame;
                frame.mData1                   = label;
                frame.mData2                   = data;
                frame.mFlags                   = 0;
            ...
            if (state == WAIT)
            {
                while (1)
                {
                    ...
                    if (transmission_error) //This is the handling of a transmission_error in case CRC is not good! Wait until we find a no data timeout period
                    {
                        if (Rx_msg)
                        {
                            ...
                            if (!mRx->WouldAdvancingCauseTransition(timeout * samples_per_bit))
                            {
                                ...
                                if (mRx->GetSampleNumber() < mTx->GetSampleNumber())
                                {                                                                                    //if we found a msg in Rx earlier Rx_msg
                    ...
                    if (!noop && !collision_detection)
                    { //Normally, in Tx msg Rx is left behind, so we advance to the next msg
                        mRx->AdvanceToNextEdge();
                    }
                    ...
                    else
                    {
                        ...
                        { //In case of a previous error if we still have data we wait
                            Rx_msg             = 0;
                            transmission_error = 1;
                            continue;
                        }
